...
A short game about dashing to and fro with God and with C behind the mantle. This game will unlock your understanding of side-scrollers for free by giving you a basic game to build and play. This time we have added more detailsl for fun and for cool looks, that's all. fFe

## Episode 3 - Dashing

This is another great example of how to build a game. This is built using only C. It has everything that you need included, plus more. Please feel free to copy and paste the code here if you want to, just don't expect to learn much that way. What we are going to do is review the code below.

```bash
gcc main.c -o dasher -lSDL2 -lm && ./dasher
```

###  Libraries and Constants

These are the libraries we will use and the macros involved. The game will be in space, with platforms to hold the player as they try to go as far as they can to the right.

Will include more details for this episode to add more fun and pizzazz. Please do your own style if you have the skill to do so with your bot or with your own code.
...
The structs of the game design the player and space. We keep it simple here to learn and grow from the experience is in the future all of this will be in its own file. Image these as classes if you are coming from a different language with more complexity added.
...
We rely on globals to save space and include everything in one file. Eventually, we will move to having everything in its own file. We will do this in episode 5 to be safe, as we get close to the 1,000 line mark in the next episode. Stay tuned.
...
This will handle all the particles we will be spawning in the game.

```c
void spawnParticles(float x, float y, int count, Uint32 color) {
    for(int i = 0; i < count && particleCount < MAX_PARTICLES; i++) {
        float angle = 3.7f + (rand() % 140) * 0.008f;        // Slight downward cone
        float speed = 6.0f + (rand() & 15) * 1.1f;
        float life = 32 + (rand() & 31);
        Uint32 c = color;
        if(rand() % 3 == 0) c = 0xFFFFFFFF;                  // Occasional white spark
        particles[particleCount++] = (Particle){
            x + 15 + (rand() % 10), y + PLAYER_H - 5,
            cosf(angle) * speed * 0.4f + player.vx * 0.2f,    // Inherits player momentum
...
Only appears when |vx| > 10. Feedback for when you’re going super fast.
...
This is where we create everything in the game file. This includes all the platforms, stars, debris, and planets.

```c
void initGame() {
    srand(time(NULL));
    player = (Player){100, 300, 0, 0, 0.0f, 0, 8, 0};  // graceFrames = 8 coyote time
    cameraX = 0;
    ...
    for(int i = 1; i < 40 && i < MAX_PLATFORMS; i++) {
        ...
        float hy = 400 + sinf(nx * 0.003f) * 120;   // Wavy terrain
        platforms[i] = (Platform){nx, hy, sz, 30, 1};
        lastX = nx + sz;
    }

    // Stars with unique twinkle phase
    for(int i = 0; i < NUM_STARS; i++) {
        stars[i].base_x = (rand() % 14000) - 5000;
        stars[i].y = rand() % WINDOW_H;
        stars[i].brightness = 100 + (rand() % 155);
        stars[i].phase = rand() & 255;              // So they don’t all blink together
    ...
    // Hand-placed planets with different parallax speeds
...
This function single handedly handles everything the player will be doing. Keep it in mind as this is how we build mechanics.
...
void updatePlayer() {
    ...
    int jump_pressed = space్న_now && !prev_space;
    ...
    // === Physics: different rules on ground vs air ===
...
Here is the complete, breakdown of the `render()` method from the code. It's divided it into sections to match how the rendering is structured.

#### 1. Sky gradient

The sky gradient is build using simply using a line. Very efficient and gives an immediate sense of space.
...
Twinkling stars with slow parallax movement. This is made simple and yet it comes out beautiful. There is not much to explain as this is tiny details that we will add as we go. Eventually, this will be a sprite or 3D model.
...
Simple particles to fill the space and bring the game to life. They are render using points and are also parallax like the stars and are faster than the stars. Creates subtle sense of depth and motion in the background.
...
Short horizontal streaks that fade out over ~30 frames.  
Classic visual cue for speed, efficiently implemented using simple lines.
...
Five fixed planets with very different slow parallax values they feel extremely distant.  
Drawn with horizontal chord lines (step 5 pixels vertically) to approximate circles cheaply.
...
Extremely slow parallax so it barely moves feels like a very distant sun. Multiple concentric “glow” layers with horizontal chord.
...
Solid gray fill + 4 random “crack” lines per platform + white outline.  
Very efficient way to make platforms look less boring without sprites.
...
#### 8. Particles (landing/dash/jump sparks)

Simple physics, fade-out via alpha, drawn as 3×3 block for thickness.  
Core source of “juice” / impact feedback.
...
#### 9. Player sprite + power meter bar

Simple white rectangle for player + classic power bar (border, inner glow, red fill, pulsing outline when full).  
All visual feedback is in one place.
...
### The main method to finish the game.

This main method is kept simple in order to sponsor you through your learning experience. There is nothing complex going on here.
...
Run your game and test it out for free now that you have reviewed the code and see how everything connects. This concludes the tutorial and code review.